  ...
  <div class="chat-view solo-chat-view">
    ...
    <div class="chat-messages" id="chat-container">
      ...
      <div v-if="$store.messages.length === 0" class="empty-chat">
        ...
        <h3 class="empty-title">Solo 模式</h3>
        <p class="empty-desc">单 Agent + 全工具，适合快速问答与轻量任务</p>
      </div>
...
export default {
  name: 'SoloChatView',
  components: { ChatMessage, ModeSelector },
  setup() {
...
.solo-tag { background: #dbeafe; color: #1d4ed8; }
.workflow-badge {
...
.dots-loader span {
  ...
  background: #3b82f6; animation: dot-bounce 1.3s infinite;
}
...
@media (prefers-color-scheme: dark) {
  ...
  .solo-tag { background: #1e3a5f; color: #60a5fa; }
